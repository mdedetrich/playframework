  ...
  "JSON" should {
    "create json with rich syntax" in {
      val js = Json.obj(
        ...
        "key2" -> 123,
        "key3" -> true,
        "key4" -> Json.arr("value41", 345.6, JsString("test"), JsObject(Map("key411" -> obj("key4111" -> 987.654))))
      )
      ...
      js must equalTo(
        JsObject(Map(
          ...
          "key4" -> JsArray(Vector(
            ...
            JsString("test"), JsObject(Map("key411" -> JsObject(Map("key4111" -> JsNumber(987.654)))))
          ))
        ))
      )
